...
* The migration procedure is only for an instance that was initialized under Shamir. A fresh cluster, or a wiped instance, initializes directly under the auto seal (`nordri/openbao-init.sh`) and never has unseal keys at all; the go-live design (`docs/plans/2026-09-16-openbao-go-live-design.md`) took that path on gke rather than migrating, since the vault held nothing a re-seed could not regenerate, and added the shared password safe as the second custody copy.
