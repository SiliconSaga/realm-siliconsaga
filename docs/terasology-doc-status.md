...
| Gradle version | No doc states any Gradle version. The wrapper is authoritative — read `gradle/wrapper/gradle-wrapper.properties`; it moved twice in the month this row was written |
...
| The build harness writes into module source trees | A harness build installs `templates/module.logback-test.xml` as each module's `src/test/resources/logback-test.xml`, and `templates/build.gradle` over each module's own. Modules gitignore `build.gradle`; the logback file is not yet ignored everywhere and shows up as an untracked change. Where a module is meant to differ — `Kallisti` carries a deliberately different `build.gradle` (it is half engine extension, supplying JNLua for `KComputers`) — the overwrite destroys intent |
...
| Test tasks never fail the build, on purpose | `build-logic/src/main/kotlin/terasology-metrics.gradle.kts` sets `ignoreFailures = true` inside `tasks.withType<Test>`, so every test task exits zero whatever the tests did. This is deliberate: Jenkins reads the reports and marks a build carrying analytics or test findings UNSTABLE rather than FAILED, reserving the exit code for "the build broke". The gap is local, where nothing reads the reports — a green `./gradlew :engine-tests:test`, or `ws test terasology`, means the run completed, and `engine-tests/build/test-results/**/TEST-*.xml` is the only verdict. **A local signal that separates "broke" from "has findings" without disturbing the Jenkins semantics is an open design question.** Lint differs by tool: Checkstyle sets `isIgnoreFailures = false` and does fail; PMD and SpotBugs do not. Pairs with the row above — CI never runs the game, and the tests it does run deliberately do not speak through an exit code |
