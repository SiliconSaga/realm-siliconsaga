...
| Test tasks never fail the build | `build-logic/src/main/kotlin/terasology-metrics.gradle.kts` sets `ignoreFailures = true` inside `tasks.withType<Test>`, so every test task exits zero whatever the tests did. A green `./gradlew :engine-tests:test` — or `ws test terasology` — means the run completed, not that it passed; `engine-tests/build/test-results/**/TEST-*.xml` is the only verdict. Pairs with the row above: CI never runs the game, and the tests it does run cannot report failure through an exit code. Lint differs by tool — Checkstyle sets `isIgnoreFailures = false` and does fail, PMD and SpotBugs do not |
