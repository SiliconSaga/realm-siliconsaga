...
### Source of truth: GitHub authoritative, Forgejo writable, `main` synced by an in-cluster puller
...
| Who may write `main` on Forgejo | In steady state, only the puller. Enforced, not assumed: `main` is branch-protected in Forgejo with a push allowlist of the puller's identity and the break-glass admins, so an arbitrary token cannot change the branch ArgoCD reads. Because both the puller's `--force-with-lease` and an orphan-snapshot hydration *replace* `main`, the protection also **enables force push for exactly that allowlist** (Forgejo's force-push allowlist, distinct from the push allowlist) and rejects force pushes from every other identity; the configure Job validates the rule's fields against the Forgejo version the pinned chart deploys, since these settings changed shape across Forgejo releases. The one sanctioned exception to the puller being sole writer is a human-initiated test hydration (see Local-branch testing), which runs as an allowlisted admin and suspends the puller for its duration. Every other branch is free. |
...
2. Ensures the puller's **Forgejo** access token exists, without ever minting one implicitly. The token has a fixed name in Forgejo (`puller`), which is what makes it reconcilable. Three states: the puller Secret exists and the token authenticates, keep it; neither the Secret nor a token of that name exists (first run), mint one scoped to write the listed repositories and write the Secret; anything else, such as the Secret missing while the named token exists or the token no longer authenticating, is a partial-failure state the Job **refuses to repair on its own**. It reports a `PullerTokenInvalid` condition and stops, because a token's value cannot be read back from Forgejo and silently minting another would leave untracked tokens behind. Only `--rotate puller` resolves it: revoke the named token, mint a replacement, rewrite the Secret. This is the only credential in the design apart from the passwords above, it is Forgejo-side, and it never leaves the cluster.
3. Ensures `main` branch protection with the push and force-push allowlists described under Source of truth.
...
Credentials are replaced only through an explicit `--rotate <admin|puller|admins>` flag on the Job, which for passwords regenerates the named credential in OpenBao, waits for ESO, and re-applies it in Forgejo, and for the puller token revokes by name then mints. `--rotate admins` also revokes every access token held by the break-glass accounts, since a password change alone leaves tokens valid. Rotation is documented during Phase 2 (open question 4).
...
Unchanged in shape from the May design. Pre-conditions, checked by the script before it does anything: the coordinated URL PR train is **merged on GitHub `main`** for every maintained repository in the claim; `maturity: durable` committed and hydrated; Forgejo composition Ready; both init Jobs succeeded; the puller has completed at least one run *after* the PR train merged, so every maintained repository on Forgejo reflects it; every vendor mirror reports a successful last sync and the refs Applications pin (today `keycloak-k8s-resources` tag `26.6.3`) resolve on Forgejo, checked per mirror since a puller run says nothing about them; and the gates below are green. Ordering matters: a first pull that runs before the train merges leaves seed URLs on Forgejo, and a bare equality check would not notice.

1. **Verify Forgejo holds the truth.** Each maintained repository's `main` on Forgejo matches GitHub's `main` by commit, **and** the Application manifests on Forgejo contain no `gitea-http` URL. Equality alone is not enough; the second check is what catches a pull that predates the train. Vendor mirrors are checked separately as above; they carry no repoURLs of ours.
2. **Push the swap commit into the seed.** One hydration into the seed *without* the URL patch, so every Application manifest the seed serves now points at Forgejo. Concretely, `graduate.sh` runs `update-embedded-git.sh <env> <realm> --swap`: target is the seed, and `--swap` sets `HYDRATE_URL_PATCH=off` so `hydrate_working_tree_repo` is called with the overlay and project patch functions but **not** the seed-URL rewrite. That is the third hydration mode beside `--target seed` (rewrite on) and `--target forgejo` (no rewrite), and the nordri unit test covers all three snapshots: seed has only `gitea-http`, forgejo has only `forgejo-http`, swap has only `forgejo-http` while still carrying the seed overlay patches.
...
**The workstation authenticates with a personal Forgejo token, never the admin password.** Each human who hydrates holds their own token, kept in the workspace `.env` as `FORGEJO_TOKEN`, the same place `GH_TOKEN` lives. The script passes it through a credential helper rather than embedding it in the remote URL, so it does not land in a temp dir's `.git/config` or in process arguments; today's seed path embeds the admin password in the URL, and that habit stops here. Branch protection admits the push because the account is on the `main` and force-push allowlists. The admin password itself stays in OpenBao and is read by nothing on a workstation.

**Issuance and revocation.** The human issues their own token from their break-glass account, scoped to `write:repository` only, through Forgejo's user-token API or the same form in the UI, using the account password read from OpenBao at `secret/forgejo/admins/<user>` with `bao kv get`. A nordri helper wraps the API call so the scope is not left to memory; it validates the token endpoint and scope names against the Forgejo version the pinned chart deploys, since Forgejo's token scopes were reworked and the plan re-checks them for the pinned version rather than assuming. Revocation is the same API by token name, and is part of two flows: a human offboarding, and `--rotate admins` on the configure Job, which revokes every token on the break-glass accounts. Tokens are never written to OpenBao; they live in the human's `.env` and in Forgejo's own token list, which is the audit surface.
