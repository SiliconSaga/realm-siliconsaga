...
Core principles are explained here. Each use-case pattern has its own deep-dive in [`docs/agent-security/`](agent-security/).
...
Each agent session is described on four independent axes. The full model — including why read and write external access are not meaningfully different, and how to enforce network scoping per OS and with Kubernetes — is in [advanced-capability-model.md](agent-security/advanced-capability-model.md).
...
|---------|-----|-------------|
| GitOps Staging Queue | [pattern-gitops-staging.md](agent-security/pattern-gitops-staging.md) | Thelio (ArgoCD), any dev machine |
| Calendar Management | [pattern-calendar.md](agent-security/pattern-calendar.md) | M1 Mac, Android phone |
| Email Drafting | [pattern-email.md](agent-security/pattern-email.md) | M1 Mac |
| Contact Management | [pattern-contact-management.md](agent-security/pattern-contact-management.md) | M1 Mac, Intel Mac (staging) |
| Chat Segmentation | [pattern-chat-segmentation.md](agent-security/pattern-chat-segmentation.md) | Thelio (Matrix), Intel Mac (Discord) |
| Voice-to-Action Pipeline | [pattern-voice-pipeline.md](agent-security/pattern-voice-pipeline.md) | Android phone → M1 Mac |
| OpenClaw Security Guide | [openclaw-security.md](agent-security/openclaw-security.md) | M1 Mac, Intel Mac |
| Advanced Capability Model | [advanced-capability-model.md](agent-security/advanced-capability-model.md) | All machines, Thelio (K8s) |
| Implementation Phases | [implementation-phases.md](agent-security/implementation-phases.md) | Phased roadmap, Phase 1 worked example |
